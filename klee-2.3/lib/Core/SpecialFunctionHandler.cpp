...
#define addDNR(name, handler) { name, \
  ...
  // klokkos additions
  // don't forget to add to main to silence warnings on startup
  add("kokkos_initialize", handleKokkosInitialize, false),
  add("kokkos_finalize", handleKokkosFinalize, false),

  add("on_view_create", handleOnViewCreate, false),
  add("on_view_mirror", handleOnViewMirror, false),
  add("on_view_reference", handleOnViewReference, false),
  add("on_deep_copy", handleOnDeepCopy, false),
  add("for_initialize", handleForInitialize, false),
  add("for_finalize", handleForFinalize, false),
  add("view_deep_copy", handleViewDeepCopy, false),
#undef addDNR
#undef add
};
...
void SpecialFunctionHandler::handleKokkosInitialize(ExecutionState &state,
                                                    KInstruction *target,
                                                    std::vector<ref<Expr> > &arguments) {
  if (arguments.size() != 1) {
    executor.terminateStateOnUserError(state, "Incorrect number of arguments to kokkos_initialize");
    return;
  }
  state.kdefault_space = arguments[0];
  // TODO magic number from Kokkos_Core
  // HOST_SPACE
  state.k_espace = ConstantExpr::alloc(0, 32);
}

void SpecialFunctionHandler::handleKokkosFinalize(ExecutionState &state,
                                                  KInstruction *target,
                                                  std::vector<ref<Expr> > &arguments) {
...
void SpecialFunctionHandler::handleOnViewCreate(ExecutionState &state,
                                                KInstruction *target,
                                                std::vector<ref<Expr> > &arguments) {
  if (arguments.size() != 3) {
    executor.terminateStateOnUserError(state, "Incorrect number of arguments to add_view_expr");
    return;
  }

  auto expr_id = dyn_cast<ConstantExpr>(executor.toUnique(state, arguments[0]));
  if (!expr_id) {
    executor.terminateStateOnUserError(state, "Could not determine view id");
    return;
  }
  executor.onViewCreate(state, expr_id->getZExtValue(32), arguments[1], arguments[2], target);
}

void SpecialFunctionHandler::handleOnViewMirror(ExecutionState &state,
                                                KInstruction *target,
                                                std::vector<ref<Expr> > &arguments) {
  if (arguments.size() != 2) {
    executor.terminateStateOnUserError(state, "Incorrect number of arguments to add_view_expr");
  ...
  auto expr_host = dyn_cast<ConstantExpr>(executor.toUnique(state, arguments[1]));
  if (!expr_host) {
    executor.terminateStateOnUserError(state, "Could not determine mirror id");
    return;
  }
  executor.onViewMirror(state, expr_id->getZExtValue(32), expr_host->getZExtValue(32), target);
}

void SpecialFunctionHandler::handleOnViewReference(ExecutionState &state,
                                                   KInstruction *target,
                                                   std::vector<ref<Expr> > &arguments) {
  if (arguments.size() != 2) {
    executor.terminateStateOnUserError(state, "Incorrect number of arguments to add_view_expr");
  ...
  executor.onViewReference(state, expr_id->getZExtValue(32), arguments[1], target);
}

void SpecialFunctionHandler::handleOnDeepCopy(ExecutionState &state,
                                              KInstruction *target,
                                              std::vector<ref<Expr> > &arguments) {
  if (arguments.size() != 2) {
    executor.terminateStateOnUserError(state, "Incorrect number of arguments to add_view_expr");
  ...
  auto expr_dst = dyn_cast<ConstantExpr>(executor.toUnique(state, arguments[0]));
  if (!expr_dst) {
    executor.terminateStateOnUserError(state, "Could not determine dst id");
    return;
  }

  auto expr_src = dyn_cast<ConstantExpr>(executor.toUnique(state, arguments[1]));
  if (!expr_src) {
    executor.terminateStateOnUserError(state, "Could not determine src id");
    return;
  }
  executor.onDeepCopy(state, expr_dst->getZExtValue(32), expr_src->getZExtValue(32), target);
}

void SpecialFunctionHandler::handleForInitialize(ExecutionState &state,
                                                 KInstruction *target,
                                                 std::vector<ref<Expr> > &arguments) {
  if (arguments.size() != 5) {
    executor.terminateStateOnUserError(state, "Incorrect number of arguments to for_initialize");
    return;
  }
  auto idx1_ptr = dyn_cast<ConstantExpr>(executor.toUnique(state, arguments[1]));
  if (!idx1_ptr) {
    executor.terminateStateOnUserError(state, "non-const ptr to idx1");
    return;
  }
  ObjectPair op1;
  if (!state.addressSpace.resolveOne(idx1_ptr, op1)) {
    executor.terminateStateOnUserError(state, "unable to resolve idx1");
    return;
  }
  executor.executeMakeSymbolic(state, op1.first, "idx1");

  auto idx2_ptr = dyn_cast<ConstantExpr>(executor.toUnique(state, arguments[2]));
  if (!idx2_ptr) {
    executor.terminateStateOnUserError(state, "non-const ptr to idx1");
    return;
  }
  ObjectPair op2;
  if (!state.addressSpace.resolveOne(idx2_ptr, op2)) {
    executor.terminateStateOnUserError(state, "unable to resolve idx2");
    return;
  }
  executor.executeMakeSymbolic(state, op2.first, "idx2");

  executor.forInitialize(state, arguments[0], arguments[3], arguments[4], op1.first, op2.first);
}

void SpecialFunctionHandler::handleForFinalize(ExecutionState &state,
                                               KInstruction *target,
                                               std::vector<ref<Expr> > &arguments) {

  if (arguments.size() != 0) {
    executor.terminateStateOnUserError(state, "Incorrect number of arguments to for_finalize");
    return;
  }
  executor.forFinalize(state);
}

void SpecialFunctionHandler::handleViewDeepCopy(ExecutionState &state,
                                               KInstruction *target,
                                               std::vector<ref<Expr> > &arguments) {
  if (arguments.size() != 2) {
    executor.terminateStateOnUserError(state, "Incorrect number of arguments to deep_copy");
    return;
  }

  executor.viewDeepCopy(state, target, arguments[0], arguments[1]);
}
