...
struct KokkosViewState {
  ref<Expr> size;
  ref<Expr> mspace;
  std::uint32_t mirror;
  bool stale;
  KokkosViewState() : size(nullptr), mspace(nullptr), mirror(0), stale(false) {};
  KokkosViewState(const KokkosViewState &src) : size(src.size), mspace(src.mspace), mirror(src.mirror), stale(src.stale) {}
};
...
public:
  ...
  /// @brief default execution space (HOST|DEVICE) for program
  ref<Expr> kdefault_space;

  /// @brief kokkos current execution space (HOST|DEVICE)
  /// Outside of parallel for loops, his is equal to kdefault_space
  ref<Expr> k_espace;

  /// @brief 1st of range used for kokkos parallel for
  ref<Expr> kfor_beg;

  /// @brief 2nd of range used for kokkos parallel for
  ref<Expr> kfor_end;

  /// @brief 1st of symbolic indicies used for kokkos parallel for
  const MemoryObject *kfor_idx1;

  /// @brief 2nd of symbolic indicies used for kokkos parallel for
  const MemoryObject *kfor_idx2;

  /// @brief maintains a map by kokkos view id of view access operations
  std::map<uint32_t, std::set<std::pair<ref<Expr>,KInstruction*>>> kview_accesses;

  /// @brief maintains a map by kokkos view id of view write operations
  std::map<uint32_t, std::set<std::pair<ref<Expr>,KInstruction*>>> kview_writes;

  std::map<uint32_t, KokkosViewState> kview_states;
