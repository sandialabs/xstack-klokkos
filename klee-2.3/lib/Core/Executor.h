...
public:
  ...
  void forInitialize(ExecutionState &state, ref<Expr> &espace, ref<Expr> &beg, ref<Expr> &end, const MemoryObject *idx1, const MemoryObject *idx2);
  void forFinalize(ExecutionState &state);
  void onViewCreate(ExecutionState &state, uint32_t view_id, ref<Expr> &size, ref<Expr> &mspace, KInstruction *kinst);
  void onViewMirror(ExecutionState &state, uint32_t view_id, uint32_t host_id, KInstruction *kinst);
  void onViewReference(ExecutionState &state, uint32_t view_id, ref<Expr> &ref, KInstruction *kinst);
  void onDeepCopy(ExecutionState &state, uint32_t dst_id, uint32_t src_id, KInstruction *kinst);
  bool isInFor(ExecutionState &state) const { return !state.kfor_beg.isNull(); }
  void validateViewAccessExprs(ExecutionState &state) const;
  void viewDeepCopy(ExecutionState &state, KInstruction *kinst, ref<Expr> &dst_size, ref<Expr> &src_size) const;
};
