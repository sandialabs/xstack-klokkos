...
class IdxReplaceVisitor : public ExprVisitor {
private:
  ref<Expr> src, dst;

public:
  IdxReplaceVisitor(const ref<Expr> &_src, const ref<Expr> &_dst)
      : src(_src), dst(_dst) {}
...
void Executor::forInitialize(ExecutionState &state, ref<Expr> &espace, ref<Expr> &beg, ref<Expr> &end, const MemoryObject *idx1, const MemoryObject *idx2) {

  if (isInFor(state)) {
    terminateStateEarly(state, "Nested parallel for is unsupported.", StateTerminationType::MaxDepth);
  } else {
    state.kview_accesses.clear();
    state.kview_writes.clear();
    state.k_espace = espace;
    state.kfor_beg = beg;
    state.kfor_end = end;
    state.kfor_idx1 = idx1;
    state.kfor_idx2 = idx2;

    // constrain idx1 to task bounds
    const ObjectState *os1 = state.addressSpace.findObject(idx1);
    ref<Expr> eidx1 = os1->read(0, Expr::Int32);
    state.addConstraint(UleExpr::create(beg, eidx1));
    state.addConstraint(UltExpr::create(eidx1, end));

    // constrain idx2 to task bounds
    const ObjectState *os2 = state.addressSpace.findObject(idx2);
    ref<Expr> eidx2 = os2->read(0, Expr::Int32);
    state.addConstraint(UleExpr::create(beg, eidx2));
    state.addConstraint(UltExpr::create(eidx2, end));
  }
}

void Executor::forFinalize(ExecutionState &state) {

  if (isInFor(state)) {
    validateViewAccessExprs(state);
    state.kview_accesses.clear();
    state.kview_writes.clear();
    // TODO magic number from Kokkos_Core
    // HOST_SPACE
    state.k_espace = ConstantExpr::alloc(0, 32);
    state.kfor_beg = state.kfor_end = nullptr;
    state.kfor_idx1 = state.kfor_idx2 = nullptr;
  } else {
    terminateStateEarly(state, "Nested parallel for is unsupported.", StateTerminationType::MaxDepth);
  }
}

void Executor::onViewCreate(ExecutionState &state, uint32_t view_id, ref<Expr> &size, ref<Expr> &mspace, KInstruction *kinst) {

  // record the views metadata
  KokkosViewState &vstate = state.kview_states[view_id];
  vstate.size = size;
  vstate.mspace = mspace;
  // outs() << "onViewCreate: " << view_id << " " << size << " " << mspace << "\n";
}

void Executor::onViewMirror(ExecutionState &state, uint32_t view_id, uint32_t host_id, KInstruction *kinst) {

  // lookup the views and flag mirrors
  auto view = state.kview_states.find(view_id);
  if (view != state.kview_states.end()) {
    auto host = state.kview_states.find(host_id);
    if (host != state.kview_states.end()) {
      // reject multiple mirrors as of now
      if (host->second.mirror) {
        terminateStateEarly(state, "Nested parallel for is unsupported.", StateTerminationType::MaxDepth);
      }

      view->second.mirror = host_id;
      host->second.mirror = view_id;
      // outs() << "onViewMirror: " << view_id << " " << host_id << "\n";
    }
  }
}

void Executor::onDeepCopy(ExecutionState &state, uint32_t dst_id, uint32_t src_id, KInstruction *kinst) {

  // lookup the views
  auto dst_itr = state.kview_states.find(dst_id);
  if (dst_itr != state.kview_states.end()) {
    auto src_itr = state.kview_states.find(src_id);
    if (src_itr != state.kview_states.end()) {
      KokkosViewState &dst = dst_itr->second;
      KokkosViewState &src = src_itr->second;
      // ...
      // outs() << "onDeepCopy: " << dst_id << " " << src_id << "\n";
      if (dst_id == src.mirror) {
        dst.stale = false;
        // outs() << "onDeepCopy cleared stale: " << dst_id << "\n";
      }
    }
  }
}

void Executor::onViewReference(ExecutionState &state, uint32_t view_id, ref<Expr> &expr, KInstruction *kinst) {

  // lookup this view in map
  auto itr = state.kview_states.find(view_id);
  if (itr != state.kview_states.end()) {
    KokkosViewState &view = itr->second;

    // check if this offset expression could be out of bounds
    if (isInFor(state)) {
      const ObjectState *os1 = state.addressSpace.findObject(state.kfor_idx1);
      ref<Expr> idx1 = os1->read(0, Expr::Int32);
      ref<Expr> inbounds = UltExpr::create(idx1, view.size);

      bool result;
      bool success = solver->mayBeFalse(state.constraints, inbounds, result, state.queryMetaData);
      if (success && result) {
        std::string str;
        llvm::raw_string_ostream os(str);

        os << "Error: kokkos view index out of bounds, view_id: " << view_id
           << " at " << state.stack.back().caller->getSourceLocation();
        klee_warning("%s", os.str().c_str());
        interpreterHandler->processTestCase(state, os.str().c_str(), "kokkos.err");
      }
    }

    // record the view access expressions.  this could be
    // consoldiated into view struct, but have different lifetimes
    if (!state.stack.empty()) {
      StackFrame sf = state.stack.back();
      bool is_write = kmodule->isKlokkosWriteToView(sf.caller->inst);
      if (isInFor(state)) {
        state.kview_accesses[view_id].insert(std::make_pair(expr, sf.caller));
        if (is_write) {
          state.kview_writes[view_id].insert(std::make_pair(expr, sf.caller));
        }
      }

      if (is_write) {

        // written to the view.  if this view has a mirror,
        // then its now stale
        if (view.mirror != 0) {
          auto itr2 = state.kview_states.find(view.mirror);
          if (itr2 != state.kview_states.end()) {
            itr2->second.stale = true;
            // outs() << "marked view " << mirror << " stale.\n";
          }
        }
      } else {
        // read from the view.
        if (view.stale) {
          ...
          os << "kokkos view read from potential stale"
            << " view_id: " << view_id
            << " at " << state.stack.back().caller->getSourceLocation();
          klee_warning("%s", os.str().c_str());
          interpreterHandler->processTestCase(state, os.str().c_str(), "kokkos.err");
    ...
    // Check mem & exec space match
    ref<Expr> space_match_query = EqExpr::create(view.mspace, state.k_espace);
    bool result;
    bool success = solver->mayBeFalse(state.constraints, space_match_query, result, state.queryMetaData);
    if (success && result) {
      std::string str;
      llvm::raw_string_ostream os(str);

      os << "Error: kokkos view could be inaccessible from execution space at "
         << state.stack.back().caller->getSourceLocation();
      klee_warning("%s", os.str().c_str());
      interpreterHandler->processTestCase(state, os.str().c_str(), "kokkos.err");
    }
  }
...
void Executor::validateViewAccessExprs(ExecutionState &state) const {

  assert(isInFor(state));

  const ObjectState *os1 = state.addressSpace.findObject(state.kfor_idx1);
  const ObjectState *os2 = state.addressSpace.findObject(state.kfor_idx2);

  assert(os1 && os2);
  ref<Expr> idx1 = os1->read(0, Expr::Int32);
  ref<Expr> idx2 = os2->read(0, Expr::Int32);

  for (auto pr: state.kview_writes) {

    // for each write to this view, check for a potentially
    // conflicting access
    for (auto &write: pr.second) {

      IdxReplaceVisitor replace(idx1, idx2);
      ref<Expr> rewrite = replace.visit(write.first);
      for (auto &access : state.kview_accesses[pr.first]) {

        ref<Expr> query = AndExpr::create(NeExpr::create(idx1, idx2), EqExpr::create(rewrite, access.first));

        bool result;
        bool success = solver->mayBeTrue(state.constraints, query, result, state.queryMetaData);
        if (success && result) {

          std::string str;
          llvm::raw_string_ostream os(str);

          os << "Error: kokkos view write offset conflicts with parallel read offset with"
             << " view_id: " << pr.first
             << " write at: " << write.second->getSourceLocation()
             << " read at: " << access.second->getSourceLocation();
          klee_warning("%s", os.str().c_str());
          interpreterHandler->processTestCase(state, os.str().c_str(), "kokkos.err");
        }
      }
...
void Executor::viewDeepCopy(ExecutionState &state, KInstruction *kinst, ref<Expr> &dst_size, ref<Expr> &src_size) const {
    ref<Expr> query = EqExpr::create(dst_size, src_size);
    bool result;

    // Check if sizes are guaranteed to mismatch
    bool success = solver->mustBeFalse(state.constraints, query, result, state.queryMetaData);
    if (success && result) {
        std::string str;
        llvm::raw_string_ostream os(str);

        os << "Error: kokkos deep_copy view dimensions mismatch at "
           << state.stack.back().caller->getSourceLocation();
        klee_warning("%s", os.str().c_str());
        interpreterHandler->processTestCase(state, os.str().c_str(), "kokkos.err");
    } else {
        // Also warn if there may be a size mismatch
        success = solver->mayBeFalse(state.constraints, query, result, state.queryMetaData);
        if (success && result) {
            std::string str;
            llvm::raw_string_ostream os(str);

            os << "Warning: kokkos deep_copy view dimensions may differ at "
               << state.stack.back().caller->getSourceLocation();
            klee_warning("%s", os.str().c_str());
            interpreterHandler->processTestCase(state, os.str().c_str(), "kokkos.err");
        }
