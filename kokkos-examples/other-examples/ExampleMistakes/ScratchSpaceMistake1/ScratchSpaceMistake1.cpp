...
int main( int argc, char *argv[] )
{
    ...
    Kokkos::initialize( argc, argv );
    {
        ...
        // We declare ScratchViewType (doube 1D, DefExecSpace, Unmanaged Mem)
        ...
        // Get the size of scratch memory (per team)
        int scratch_size = ScratchViewType::shmem_size( M ); // Translated to byte
        ...
        for ( int repeat = 0; repeat < nrepeat; repeat++ ) {
            ...
            // Correct code allocate the scratch memory per team of "scratch_size".  Code Mistake can be either PerThread or 
            // putting smaller size than scratch
#ifdef WITH_BUG
            // This bug is simple.  The size declared in the team_policy.set_scratch_size does not match with the the ScratchView delcaration 
            // The user forget to use the value from shmem_size() call
            Kokkos::parallel_reduce( "yAx", team_policy( E, Kokkos::AUTO, 32 ).set_scratch_size( 0, Kokkos::PerTeam( M ) ),
