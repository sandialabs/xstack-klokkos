...
int main( int argc, char *argv[] )
{
    ...
    Kokkos::initialize(argc,argv);
    {
        ...
        if (argc != 2) {
            ...
            Kokkos::View<double *> C_device("B_device", N); // Allocated in the Default Memory Space
            auto C_host = Kokkos::create_mirror_view(C_device);
            ...
            Kokkos::parallel_for(Kokkos::RangePolicy<Kokkos::DefaultExecutionSpace>(0, N), KOKKOS_LAMBDA(const size_t index) {
                ...
                C_device(index) = A_device(index);  // C_device is altered
            });
#ifndef WITH_BUG
            ...
            Kokkos::parallel_for(Kokkos::RangePolicy<Kokkos::Serial>(0, N), KOKKOS_LAMBDA(const size_t index) {
                ...
                // B_host will be altered based on the value in A_host (A_host is copied from A_device)
            });
            ...
            // Execution on Default Space (it's device by default )
            Kokkos::parallel_for(Kokkos::RangePolicy<Kokkos::DefaultExecutionSpace>(0, N), KOKKOS_LAMBDA(const size_t index) {
                A_device(index) = A_device(index)-C_device(index)*2.0; // A_device is altered
            });
...
#endif
            Kokkos::parallel_for(Kokkos::RangePolicy<Kokkos::DefaultExecutionSpace>(0, N), KOKKOS_LAMBDA(const size_t index) {
                B_device(index) = B_device(index)+A_device(index)+C_device(index);
            });

            Kokkos::deep_copy(C_host, C_device);
            Kokkos::deep_copy(B_host, B_device);
            Kokkos::deep_copy(A_host, A_device);

            for (int i = 0; i < N; ++i ) {
                std::cout << "A_host( " << i << " ) = " << A_host(i) << " : B_host( " << i << " ) = " 
                << B_host(i) <<  " : C_host( " << i << " ) = " << C_host(i) <<  std::endl; 
            }
        }
    }
