...
#ifndef EXE_SPACE
#define EXE_SPACE Serial
#endif

// multiply two matrices A.X 

Kokkos::View<double *,Kokkos::EXE_SPACE::memory_space> computeMatrixMulSerial(Kokkos::View<double **,Kokkos::EXE_SPACE::memory_space> A_d, Kokkos::View<double *,Kokkos::EXE_SPACE::memory_space> B_d, int nRows, int nCols)
{
	...
	ofstream outdata;
        outdata.open("../serial.log", std::ios_base::app);	
...
void mulMatSerial(Kokkos::View<double **,Kokkos::EXE_SPACE::memory_space> A_h, Kokkos::View<double *,Kokkos::EXE_SPACE::memory_space> B_h, int nRows, int nCols)
{
    ofstream outdata;
    outdata.open("../serial.log", std::ios_base::app);	
    if( !outdata ) { // file couldn't be opened
    ...
    Kokkos::View<double **, Kokkos::EXE_SPACE::memory_space> rslt("MAT_A_times_B",nRows,1);
...
void computeDotSerial(Kokkos::View<double *,Kokkos::EXE_SPACE::memory_space> A_h, Kokkos::View<double *,Kokkos::EXE_SPACE::memory_space> B_h, int nRows)
{
...
void initializeViewsSerial(Kokkos::View<double **, Kokkos::EXE_SPACE::memory_space> &A_h, Kokkos::View<double *, Kokkos::EXE_SPACE::memory_space> &x_h, Kokkos::View<double *, Kokkos::EXE_SPACE::memory_space> &y_h, int nRows, int nCols, double lower_bound, double upper_bound){
...
bool isASCIISerial(const std::string& s)
{
...
bool is_numberSerial(const std::string& s)
{
...
int launchSerial( int argc, char *argv[] )
{
	...
	// Vector of string to save tokens
    	...
    	{
		if (!is_numberSerial(intermediate)){
			outdata.close();
                        return 1;
                }
	...
	Kokkos::initialize(argc,argv);
	{
		...
		initializeViewsSerial(A_h, x_h, y_h, nRows, nCols, lower_bound, upper_bound);

		//mulMat(A_h, x_h, nRows, nCols);	

		Kokkos::deep_copy(A_h, A_d);
		Kokkos::deep_copy(x_h, x_d);
		
		auto tmp_h = computeMatrixMulSerial(A_d, x_d, nRows, nCols);
		computeDotSerial(tmp_h, y_h, nRows);
	}
