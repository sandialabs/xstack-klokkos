...
Kokkos::View<double *,Kokkos::EXE_SPACE::memory_space> computeMatrixMul(Kokkos::View<double **,Kokkos::EXE_SPACE::memory_space> A_d, Kokkos::View<double *,Kokkos::EXE_SPACE::memory_space> B_d, int nRows, int nCols)
{
	...
	Kokkos::Profiling::popRegion();
 	ofstream outdata;
	outdata.open("../matmul.log", std::ios_base::app);
	if( !outdata ) { // file couldn't be opened
		cerr << "Error: file could not be opened" << endl;
		exit(1);
	}
	Kokkos::deep_copy(tmp_h,tmp_d);
	outdata << "Multiplication of given two matrices ("<< nRows << "x" << nCols << ").(" << nCols << "x1)" << " using kokkos constructs is:\n";
...
void computeDot(Kokkos::View<double *,Kokkos::EXE_SPACE::memory_space> A_h, Kokkos::View<double *,Kokkos::EXE_SPACE::memory_space> B_h, int nRows)
{
	...
	KOKKOS_LAMBDA(const size_t &iRow, double & valueToUpdate )
	{
		...
		//	B_h(iRow+1) = B_h(iRow%nRows)-1;    // cuda behaves similar to serial execution, change 7 to 5
		}
		#endif
		valueToUpdate += A_h(iRow) * B_h(iRow);
	},result);

	B_d = Kokkos::create_mirror_view(B_h);
	Kokkos::deep_copy(B_d,B_h);
...
void initializeViews(auto &A_h, auto &x_h, auto &y_h, int nRows, int nCols, double lower_bound, double upper_bound){
...
int main( int argc, char *argv[] )
{
 	...
 	Kokkos::initialize(argc, argv);
	{
		...
		initializeViews(A_d, x_d, y_d, nRows, nCols, lower_bound, upper_bound);
		Kokkos::deep_copy(A_h, A_d);
		Kokkos::deep_copy(x_h, x_d);
		Kokkos::deep_copy(y_h, y_d);

		tmp_h = computeMatrixMul(A_h, x_h, nRows, nCols);
		computeDot(tmp_h, y_h, nRows);
	}
