  ...
  initialize(argc,argv);
  {
    ...
    // } );
    {
      ...
      {  
        int indices[1];
        indices[0] = i;
        KokkosViewAssign(A,1,indices,i);
      };
      MyFunc(i);
    ...
    // May not need to say the execution space (maintain in a stack to indicate this is matching call). 
    {
      ...
      // should internally (in KLEE) keep the table for individual parallel_for
      int i;
      // The code generator automatically creates the next two statements based on the values passed to the range policy.
